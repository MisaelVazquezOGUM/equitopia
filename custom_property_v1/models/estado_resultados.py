...
class Estado_resultados(models.Model):
		...
		if self.property_id:
			...
			if self._origin.id:
				if self.expense_table_ids.ids:
					self.env['expense.table'].go_update(self.property_id.id, self._origin.id)
					self.env['expense.table'].go_create(self.property_id.id, self._origin.id)
				else:
					self.env['expense.table'].go_create(self.property_id.id, self._origin.id)
	...
	def generate_invoice_to_supplier(self):
		...
		if self.property_id:
			invoice_sup_dict = {
				'date': today,
				'description':'Pago de Comisión de rentas del Mes de '+ self.mes_estado,
				'property_id': self.property_id.id or False,
				'agent':self.property_id.property_manager.id or False,
				'commission_type':'fixed',
				'amount_total':self.ingresos_netos * (self.property_id.commission_percentage/100) or 0.0,
