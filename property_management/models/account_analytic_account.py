...
class AccountAnalyticAccount(models.Model):
    ...
    phone = fields.Char(string="Phone", related='tenant_id.phone')
    email_tenant = fields.Char(string="Email", related='tenant_id.email')
    ...
    # @api.model
    # def _default_set_new_company_id(self):
    #     return self.company_id

    #company2_id = fields.Many2one('res.company', string='Management Company', default=_default_set_new_company_id)
    company2_id = fields.Many2one('res.company', string='Management Company', default=lambda self: self.env['res.company'].search([('partner_id', '=', self.env.user.company_id.id)]))
            ...
            if vals.get('is_property'):
                prop_brw.write(
                    {'current_tenant_id': vals.get('tenant_id', False),
                     'state': 'normal'})
            prop_id = self.env['account.asset.asset'].search([('id','=',vals['property_id'])],limit=1)
            if prop_id:
                acces_dev = prop_id.entrega_acceso_id
                if acces_dev:
                    vals['entrega_acceso_id'] = acces_dev.id
                check_out_str = vals.get('chech_out').split(' ')
                check_out_str = check_out_str[0].split('-')
                check_out_str = [int(x) for x in check_out_str]
                check_out = date(check_out_str[0],check_out_str[1],check_out_str[2])
                check_in_str = vals.get('chech_in').split(' ')
                check_in_str = check_in_str[0].split('-')
                check_in_str = [int(x) for x in check_in_str]
                check_in = date(check_in_str[0],check_in_str[1],check_in_str[2])
                day_diff=(check_out-check_in).days
                tarifa_select=self.buscar_rango_not_self(day_diff,vals.get('property_id'),vals.get('tipo_tarifa'))
                if tarifa_select:
                    renta_total=tarifa_select.costo_tarifa*day_diff
                    pago_real=renta_total/tarifa_select.fecuencia_de_pagos
                    vals['rent'] = pago_real
                    vals['deposit'] = tarifa_select.deposito
    ...
    def write(self, vals):
        ...
        prop_id = self.env['account.asset.asset'].search([('id','=',self.property_id.id)],limit=1)
        _logger.info(prop_id)
        if prop_id:
            acces_dev = prop_id.entrega_acceso_id
            _logger.info(acces_dev)
            if acces_dev:
                vals.update({'entrega_acceso_id':acces_dev.id})
    ...
    def button_receive(self):
        ...
        for tenancy_rec in self:
            ...
            vals = {
                ...
                'communication': 'Deposit Received/Deposito Recibido',
                'tenancy_id': tenancy_rec.id,
    ...
    def edit_rent_schedules(self):
        _logger.info(f'___________Modificacion de rent schedules ids___________')
