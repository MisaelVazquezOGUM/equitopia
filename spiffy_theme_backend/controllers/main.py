...
class BackendConfigration(http.Controller):
    ...
    def get_selected_pallet(self, **kw):
        ...
        if app_light_bg_image:
            if 'data:image/' in str(app_light_bg_image):
                ...
                app_light_bg_image = app_light_bg_image
    ...
    def get_tab_title(self, **kw):
        record_vals = request.env['res.config.settings'].sudo().search([], order="id desc", limit=1)
        new_name = record_vals.tab_name
        return new_name
    ...
    def get_irmenu_icondata(self, **kw):
        ...
        irmenu = request.env['ir.ui.menu'].sudo().search([('id','=',kw.get('menu_id'))])
        for menu in irmenu:
            menu_dict = menu.read(list(set(irmenuobj._fields)))
            return menu_dict