...
class ExpenseTable(models.Model):
    ...
    def go_create(self,prop_id,stat_id,fecha):
        self.get_records(stat_id,prop_id,fecha)
    
    def go_update(self,prop_id,stat_id,fecha):
    ...
    '''
    def get_records(self,state_result_id,prop_id,fecha):
        vals_ot_exp = {}
        exp = []
        fecha_actual=fecha
        ...
        comisiones=contratos.search([('property_id','=',prop_id),('commission','=',True),
                ('chech_in','>=',date_start),('chech_in','<=',date_stop)])
        
        if comisiones:
            for comm in comisiones:
                vals_ot_exp = {
                    'state_result_id':state_result_id,
                    'date_exp':comm.chech_in,
                    'name_exp':comm.name,
                    'type_move':'co', 
                    'enter_pay':0.0,
                    'exit_pay':float(comm.total_commission),
                    'invoice_id':False
                }
                exp.append(vals_ot_exp)
