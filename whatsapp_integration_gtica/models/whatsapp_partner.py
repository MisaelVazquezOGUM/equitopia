...
class SendWhatsappPartner(models.TransientModel):
    ...
    def _onchange_price_list(self):
        if self.price_list:
            ...
            data = self.env['res.partner'].price_list_report(self.price_list, qty, product_categories_ids)
    ...
    def _onchange_message(self):
        ...
        partner_uid = self.env['res.partner'].search([('id', '=', self._context.get('uid'))])
        partner_record = self.env['res.partner'].search([('id', '=', self._context.get('active_id'))])
        ...
        # raise ValidationError( self._context.get('uid'))
